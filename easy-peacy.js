	...
	switch (req.url) {
		...
		case '/':
			...
			movies.forEach(function(movies){
				titulos.push(movies.title)
			})

			titulos.sort(); //el metodo .sort ordena alfabeticamente los datos de la variable titulo
		...
		case '/en-cartelera':
			...
			movies.forEach(function(movie){
				res.write(`->> ${movies.title.toUpperCase()} <<-`);
				res.write('\n\n')
				res.write('Reseña:')
				res.write('\n\n')
		...
		case '/mas-votadas':
			...
			res.write(`\n\n Ranting promedio ${promedio.toFixed(2)}\n\n`);
			res.write('Listado de peliculas:\n\n');
			res.write('\n\n')
			votadas.forEach(function(movie){
				...
				res.write(`Rating: ${movie.overview}\n\n\n`)
			})
			res.end();
			break;
		...
		case '/contacto':
			...
			res.write('Tenes algo para contarnos? Nos encantaria leer tus propuestas!!')
			res.end();
			break;
		...
		case '/preguntas-frecuentes':
			...
			faqs.forEach(function(preguntas){
				res.write('Preguntas: ' + preguntas.faq_title);
				res.write('\n\n')
